class Task {
    ...
    findRestTrialNum() {
        if (this.restN == 0) {
            ...
            let trial_count_before_rest = Math.floor(
                this.totalTrialN / (this.restN + 1)
            );
            this.restTrialNum = range(1, this.restN + 1).map(function (x) {
                return x * trial_count_before_rest;
            });
        }
    }
    ...
    updateProgress() {
        this.progress = Math.round(
            (100 * (this.trialNum + this.pracTrialN)) /
                (this.trialN + this.pracTrialN)
        );
    }
    ...
    rest() {
        this.restTextElement.html(
            "You are done with " +
                this.progress +
                "% of the study!<br /><br />Take a short break now and hit space to continue whenever you are ready."
        );
